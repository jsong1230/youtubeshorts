...
def collect_topics():
    ...
    if config.OPENAI_API_KEY:
        try:
            ...
            script_generator.openai_client = OpenAI(api_key=config.OPENAI_API_KEY, timeout=30.0)
        except:
            pass
    ...
    if config.CLAUDE_API_KEY:
        try:
            ...
            script_generator.claude_client = Anthropic(api_key=config.CLAUDE_API_KEY, timeout=30.0)
        except:
            pass
    ...
    if not interrupted:
        ...
        try:
            with ThreadPoolExecutor(max_workers=1) as executor:
                future = executor.submit(
                    script_generator._get_high_performing_topics,
                    content_type
                )
...
def main():
    ...
    try:
        ...
        print()
        print("=" * 60)
        print(f"🎲 랜덤 선택된 주제 {selected_count}개 (총 소요 시간: {total_elapsed:.1f}초):")
        print("=" * 60)

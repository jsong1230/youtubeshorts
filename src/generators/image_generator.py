...
class ImageGenerator:
    ...
    def prepare_thumbnail_canvas(
        self, thumbnail_path: str, target_size: Tuple[int, int]
    ) -> Optional[str]:
        ...
        try:
            ...
            temp_path = os.path.join(
                settings.TEMP_DIR, f"thumb_canvas_{int(time.time()*1000)}.jpg"
            )
    ...
    def embed_thumbnail_frame(
        self, video_path: str, thumbnail_path: str, duration: float = 0.6
    ) -> str:
        """생성된 썸네일을 영상의 첫 프레임으로 삽입."""
        if not video_path or not os.path.exists(video_path):
            logger.warning(
                "⚠️ 영상 파일을 찾을 수 없어 썸네일 프레임을 삽입하지 않습니다."
            )
            return video_path
        ...
        try:
            ...
            canvas_path = self.prepare_thumbnail_canvas(thumbnail_path, target_size)
            if not canvas_path:
                return video_path

            intro_clip = (
                ImageClip(canvas_path)
            ...
            temp_output = os.path.join(
                settings.TEMP_DIR, f"with_thumb_{int(time.time()*1000)}.mp4"
            )
            temp_audio = os.path.join(
                settings.TEMP_DIR, f"with_thumb_audio_{int(time.time()*1000)}.m4a"
            )
    ...
    def _generate_dalle3_thumbnail(
        self, title: str, topic: str = None, script: list = None, language: str = "ko"
        ...
        try:
            ...
            if any(
                k in lower_topic or k in lower_title
                for k in [
                    "money",
                    "finance",
                    "rich",
            ...
            prompt = (
                f"A viral YouTube Shorts thumbnail image for a video titled: '{title}'."
            )
            if topic:
            ...
            logger.info("🎨 DALL-E 3로 썸네일 이미지 생성 중...")
            ...
            logger.info("✅ DALL-E 3 썸네일 이미지 생성 완료")
            return img

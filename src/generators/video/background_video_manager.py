...
class BackgroundVideoManager:
    ...
    def prepare_background_clips(
        ...
        for i in range(0, len(script), group_size):
            ...
            if not bg_video_path:
                raise ValueError(f"그룹 {i+1}-{group_end}에 대한 배경 영상을 다운로드할 수 없습니다.")
            
            background_groups.append((i, group_end, bg_video_path, None))
            logger.debug(f"   배경 미디어 그룹 {len(background_groups)}: 문장 {i+1}-{group_end} (영상) - {group_sentence[:30]}...)")
    ...
    def create_background_video_clip(
        ...
        """배경 영상 클립 생성"""
        try:
            logger.debug(f"   📹 배경 영상 로드 (그룹 {group_start+1}-{group_end}): {bg_video_path}")
            ...
            raise ValueError(f"그룹 {group_start+1}-{group_end}의 배경 영상을 로드할 수 없습니다: {e}")
    ...
    def _extract_keywords(self, sentence: str, topic: str) -> Tuple[str, str]:
        ...
        if topic and self.media_downloader:
            ...
            if topic_keywords:
                ...
                topic_english = self.media_downloader.translate_keyword_to_english(topic_keyword)
                logger.debug(f"🎯 주제 키워드 우선 사용: {topic} -> {topic_keyword} -> {topic_english}")
        
        sentence_keywords = self.media_downloader.extract_keywords(sentence) if self.media_downloader else []
        ...
        if sentence_keyword:
            ...
            if topic:
                keyword = topic
                english_keyword = self.media_downloader.translate_keyword_to_english(topic) if self.media_downloader else topic
            else:
                keyword = "nature"
    ...
    def _download_with_retry_strategy(
        ...
        for retry_idx, keyword in enumerate(unique_keywords[:8]):
            bg_video_path, video_id = self.download_video_for_sentence(
                sentence, index, duration, topic=topic,
    ...
    def _get_category_keywords(self, topic: str) -> List[str]:
        ...
        if any(word in topic_lower for word in ["money", "save", "invest", "budget", "finance", "wealth", "spending", "expense", "subscription", "emergency", "fund", "401k", "roth", "ira"]):
            return ["money", "finance", "investment", "savings", "budget", "wealth", "business"]
        
        # 생산성 관련
        elif any(word in topic_lower for word in ["routine", "productivity", "focus", "workspace", "morning", "habit", "automate", "ai"]):
            return ["productivity", "workspace", "morning", "routine", "focus", "office", "desk"]
        
        # 자기계발 관련
        elif any(word in topic_lower for word in ["growth", "motivation", "success", "achievement", "goal", "mindset", "transform", "change"]):
            return ["growth", "motivation", "success", "achievement", "goal", "inspiration", "mindset"]
        
        # 생활/정리 관련
        elif any(word in topic_lower for word in ["declutter", "organize", "closet", "minimalism", "home", "lifestyle", "clean"]):
            return ["home", "lifestyle", "minimalism", "organization", "declutter", "interior"]
        
        # 기본 키워드
        return ["home", "lifestyle", "indoor", "cozy", "warm", "nature", "abstract", "cinematic"]

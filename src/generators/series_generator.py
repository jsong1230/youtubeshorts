...
class SeriesGenerator:
    ...
    def generate_series_topics(
        ...
        try:
            if series_type == SeriesType.SEQUENTIAL:
                return self._generate_sequential_series(
                    main_topic, num_episodes, content_type
                )
            elif series_type == SeriesType.THEMATIC:
                return self._generate_thematic_series(
                    main_topic, num_episodes, content_type
                )
            elif series_type == SeriesType.TUTORIAL:
                return self._generate_tutorial_series(
                    main_topic, num_episodes, content_type
                )
            elif series_type == SeriesType.CHALLENGE:
                return self._generate_challenge_series(
                    main_topic, num_episodes, content_type
                )
            else:
                return self._generate_sequential_series(
    ...
    def _generate_sequential_series(
        self, main_topic: str, num_episodes: int, content_type: ContentType
    ) -> List[Dict]:
...
Number of episodes: {num_episodes}"""
                ...
                response = self.video_generator.openai_client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": "You are an expert at creating engaging YouTube Shorts content.",
                        },
...
Number of episodes: {num_episodes}"""
                ...
                response = self.video_generator.openai_client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": "You are an expert at creating step-by-step tutorials.",
                        },
                        {"role": "user", "content": prompt},
                    ],
                    temperature=0.7,
                ...
                for line in topics_text.split("\n"):
                    ...
                    if line and (line[0].isdigit() or "Step" in line):
                        ...
                        topic = (
                            topic.split(".", 1)[-1].strip() if "." in topic else topic
                        )
                        if topic:
                            topics.append(
                                {
...
Number of episodes: {num_episodes}"""
                ...
                response = self.video_generator.openai_client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": "You are an expert at creating engaging challenge content.",
                        },
                        {"role": "user", "content": prompt},
                    ],
                    temperature=0.8,

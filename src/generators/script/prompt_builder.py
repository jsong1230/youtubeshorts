...
class PromptBuilder:
    ...
    def build_user_prompt(
        ...
        if language == "en":
            ...
            return f"'{topic}'에 대한 YouTube Shorts 영상 스크립트를 작성해주세요. 각 문장은 3-4초 분량이며, 총 {max_sentences}개 문장으로 작성하여 약 {target_duration}초 분량이 되도록 작성해주세요 (최대 60초 제한). **중요: 모든 문장은 한국어로만 작성하세요. 영어 문장이나 영어 단어를 포함하지 마세요.** 중요한 점: 순수한 대사나 설명만 작성하고, '배경음악', '자막', '시작' 같은 제작 지시사항은 절대 포함하지 마세요. 첫 문장은 반드시 스크롤을 멈추게 하는 강력한 Hook이어야 하며, 최대한 몰입도 있게 내용을 전개하세요. **매우 중요 - 년도/날짜 언급 금지: 구체적인 년도(예: '{current_year}년', '{next_year}년' 등)를 절대 언급하지 마세요. 대신 '요즘', '최근', '지금', '앞으로', '내년에는', '새해에는' 등 일반적인 시간 표현만 사용하세요. 숫자로 된 년도는 절대 사용하지 마세요.** **구체적이고 현실적인 조언 필수: 1) 일반적이고 모호한 표현 금지: '온도 조절기를 활용하세요', '단열재를 활용하세요' → 대신 '온도를 20도로 설정하세요', '창문 틈새에 문풍지를 붙이세요'처럼 구체적인 방법과 숫자를 포함하세요. 2) 바로 실행 가능한 실용적인 팁을 제공하세요. 3) 절약 금액이나 효과를 구체적인 숫자로 제시하세요 (예: '월 5만원 절약', '연간 50만원 아낄 수 있습니다'). 4) 시간은 짧아도 상관없으니 핵심만 전달하세요.** **한국 특화 필수 - 절대 규칙: 1) 특정 서비스명/브랜드명 언급 금지: '쿠팡', '네이버', '카카오톡', '배달의민족', '요기요' 등 특정 브랜드는 절대 언급하지 마세요. 2) 대신 일반적이지만 한국 문화에 맞는 표현 사용: '온라인 쇼핑몰', '배달 앱', '간편 송금', '모바일 송금', '핸드폰 송금', '할인 쿠폰', '앱 쿠폰' 등. 3) 절대 금지 표현: '홈파티' → '집들이/회식/모임'. 4) 한국인만이 공감할 수 있는 문화적 표현과 상황을 사용하되, 특정 브랜드는 언급하지 마세요.**{short_guidance_ko}"
...
- 미래를 언급할 때는 "내년에는", "새해에는", "앞으로는", "곧" 등 일반적인 시간 표현만 사용하세요.
- 과거를 언급할 때는 "최근", "요즘", "지금까지", "이전에" 등 상대적 표현을 사용하세요.
- 현재를 언급할 때는 "지금", "현재", "요즘", "최근" 등 일반적인 표현을 사용하세요.
- **절대 숫자로 된 년도를 사용하지 마세요.**
...
- ✅ 대신 일반적이지만 한국 문화에 맞는 표현 사용:
  * "온라인 쇼핑몰", "온라인 쇼핑", "인터넷 쇼핑"
  * "배달 앱", "배달 서비스", "배달 주문"
  * "간편 송금", "모바일 송금", "핸드폰 송금"
  * "할인 쿠폰", "모바일 쿠폰", "앱 쿠폰"
  * "온라인 예약", "앱 예약", "모바일 예약"
...
**구체적 예시:**
- ✅ 좋은 예: "간편 송금으로 회비를 정산하세요"
- ❌ 나쁜 예: "카카오톡 송금으로 회비를 정산하세요" (특정 서비스명 언급 금지)
- ✅ 좋은 예: "배달 앱에서 할인 쿠폰을 챙기세요"
- ❌ 나쁜 예: "배달의민족에서 할인 쿠폰을 챙기세요" (특정 서비스명 언급 금지)
- ✅ 좋은 예: "집들이를 열어 친구들과 모이세요"
- ❌ 나쁜 예: "홈파티를 열어 친구들과 모이세요"
- ✅ 좋은 예: "온라인 쇼핑몰에서 장보기 쿠폰을 활용하세요"
- ❌ 나쁜 예: "쿠팡에서 장보기 쿠폰을 활용하세요" (특정 서비스명 언급 금지)

**구체적이고 현실적인 조언 예시:**
- ✅ 좋은 예: "온도를 20도로 설정하면 월 5만원 절약됩니다"
- ❌ 나쁜 예: "온도 조절기를 활용하세요" (너무 일반적)
- ✅ 좋은 예: "창문 틈새에 문풍지를 붙이면 열 손실을 30% 줄일 수 있습니다"
- ❌ 나쁜 예: "단열재를 활용하세요" (구체적이지 않음)
- ✅ 좋은 예: "멀티탭 스위치를 끄면 월 3만원 아낄 수 있습니다"
- ❌ 나쁜 예: "가전제품을 끄세요" (구체적이지 않음)
...
**[매우 중요 - 한국 특화 필수]**
- 반드시 한국의 문화, 정서, 사회적 맥락에 특화된 스토리를 작성하세요.
- 한국의 역사적 사례, 한국 부자들의 습관, 한국 사회의 실제 사례를 다루세요.
- 한국의 특정 금융 제도(전세, 월세, 청약, 적금, 예금 등)와 관련된 스토리를 활용하세요.
- 한국인만이 공감할 수 있는 실제 경험(전세로 집 산 이야기, 적금으로 목돈 만든 이야기 등)을 다루세요.
- 특정 서비스명/브랜드명은 언급하지 마세요. 대신 "온라인 쇼핑몰", "배달 앱", "간편 송금", "모바일 송금" 등 일반적이지만 한국 문화에 맞는 표현을 사용하세요.
- ❌ "홈파티" → ✅ "집들이", "회식", "모임"
- 일반적이거나 번역된 느낌의 스토리는 절대 사용하지 마세요.""",
                sentence_count,
            ),
            ContentType.FACT: (
...
**[매우 중요 - 한국 특화 필수]**
- 반드시 한국 통계청, 한국은행, 금융감독원 등 한국 기관의 통계를 사용하세요.
- 한국의 금융 제도(전세, 월세, 청약, 적금, 예금, 주택청약종합저축 등)와 관련된 팩트를 다루세요.
- 한국 사회의 현실(서울 평균 전세 보증금, 평균 월세, 평균 연봉, 평균 주거비 부담률 등)을 반영하세요.
- 특정 서비스명/브랜드명은 언급하지 마세요. 대신 "온라인 쇼핑몰", "배달 앱", "간편 송금", "모바일 송금" 등 일반적이지만 한국 문화에 맞는 표현을 사용하세요.
- 한국인만이 공감할 수 있는 통계와 숫자를 사용하세요.
- 일반적이거나 번역된 느낌의 팩트는 절대 사용하지 마세요.""",
                sentence_count,
            ),
            ContentType.SHORT_STORY: (
...
**[매우 중요 - 한국 특화 필수]**
- 반드시 한국인의 실제 경험을 바탕으로 한 스토리를 작성하세요.
- 한국의 특정 금융 제도(전세, 월세, 청약, 적금, 예금 등)와 관련된 성공/실패 스토리를 다루세요.
- 한국 사회의 현실을 반영한 스토리(전세로 집 산 이야기, 적금으로 목돈 만든 이야기, 부동산 투자 성공/실패 이야기 등)를 작성하세요.
- 특정 서비스명/브랜드명은 언급하지 마세요. 대신 "온라인 쇼핑몰", "배달 앱", "간편 송금", "모바일 송금" 등 일반적이지만 한국 문화에 맞는 표현을 사용하세요.
- ❌ "홈파티" → ✅ "집들이", "회식", "모임"
- 한국인만이 공감할 수 있는 상황과 경험을 다루세요.
- 일반적이거나 번역된 느낌의 스토리는 절대 사용하지 마세요.""",
                sentence_count,
            ),
            ContentType.BOOK_REVIEW: (

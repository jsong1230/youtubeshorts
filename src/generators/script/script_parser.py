...
class ScriptParser:
    ...
    def parse_script_text(self, script_text: str, max_sentences: int) -> List[str]:
        ...
        if len(filtered_sentences) < 12:
            logger.warning(
                f"⚠️ Insufficient sentences generated ({len(filtered_sentences)}). Re-checking original script."
            )
    ...
    def remove_repetitive_phrases(self, sentences: List[str]) -> List[str]:
        ...
        repetitive_phrases = {
            phrase for phrase, count in phrase_counts.items() if count >= 3 and phrase
        }

        if repetitive_phrases:
            logger.debug(
                f"⚠️ Repetitive phrases detected: {list(repetitive_phrases)[:2]}"
            )
            ...
            logger.info(
                f"✅ Repetitive phrases removed: {len(repetitive_phrases)} patterns"
            )

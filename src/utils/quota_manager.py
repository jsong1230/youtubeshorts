...
class QuotaManager:
    ...
    def __init__(self, storage_path: Optional[str] = None):
        ...
        if storage_path is None:
            storage_path = os.path.join(
                os.path.expanduser("~/.gemini/antigravity"), "quota_usage.json"
            )
    ...
    def _clean_old_data(self, data: Dict) -> Dict:
        ...
        for service, limits in self.limits.items():
            ...
            if key in data[service]:
                data[service][key] = [
                    req for req in data[service][key] if req["timestamp"] > cutoff_time
                ]
    ...
    def record_usage(self, service: str, units: int = 1):
        ...
        self.usage[service][key].append({"timestamp": now, "units": units})
    ...
    def print_usage_stats(self):
        ...
        for service, data in stats.items():
            ...
            logger.info(
                f"  Current: {current}/{limit} per {window} ({percentage:.1f}%)"
            )

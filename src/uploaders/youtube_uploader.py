    ...
    def upload_video(
        ...
        if schedule_delay_hours and schedule_delay_hours > 0:
            ...
            if scheduled_time < min_scheduled_time:
                ...
                logger.info(
                    "⚠️ 예약 시간이 최소 15분 요구사항보다 짧아서 15분 후로 조정되었습니다."
                )

            # ISO 8601 형식으로 변환 (UTC)
            scheduled_start_time = scheduled_time.strftime("%Y-%m-%dT%H:%M:%S.000Z")
            logger.info(
                f"📅 예약 업로드 설정: {schedule_delay_hours}시간 후 ({scheduled_start_time})"
            )
            ...
            if privacy_status == "public":
                ...
                logger.info(
                    "⚠️ 예약 업로드는 공개 상태로 설정할 수 없어 'unlisted'로 변경되었습니다."
                )
        ...
        try:
            ...
            if thumbnail_path:
                ...
                if os.path.exists(thumbnail_path):
                    logger.info(
                        f"   ✅ 썸네일 파일 존재 확인됨 (크기: {os.path.getsize(thumbnail_path)} bytes)"
                    )
                    try:
                    ...
                    logger.warning(
                        f"   ⚠️ 썸네일 파일을 찾을 수 없습니다: {thumbnail_path}"
                    )
                    logger.info("   영상은 정상적으로 업로드되었습니다.")
            else:
                ...
                for item in response["items"]:
                    ...
                    if published_at:
                        ...
                        if published_date == today:
                            logger.info(
                                f"✅ 오늘 이미 업로드된 영상 발견: {item['snippet']['title']}"
                            )
                            return True

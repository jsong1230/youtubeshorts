...
class VideoDatabase:
    ...
    def add_video(
        ...
        try:
            ...
            if script:
                ...
                logger.debug(
                    f"✅ 스크립트 저장됨 (길이: {len(script)}자): {script_preview}..."
                )
            else:
    ...
    def update_video_stats(
        self, video_id: str, views: int = None, likes: int = None, comments: int = None
    ) -> bool:
        """
        영상 통계 업데이트
        ...
        try:
            ...
            # 기존 데이터 가져오기
            cursor.execute(
                "SELECT views, likes, comments FROM videos WHERE video_id = ?",
                (video_id,),
            )
            row = cursor.fetchone()
    ...
    def get_top_performing_videos(
        self, limit: int = 5, days: int = 30, min_views: int = 100
    ) -> List[Dict]:
        """
        성과가 좋은 영상 조회
    ...
    def get_all_videos(
        self, limit: int = None, days: int = None, order_by: str = "upload_date"
    ) -> List[Dict]:
        """
        모든 영상 조회 (필터 없이)
    ...
    def get_top_prompts(self, limit: int = 3, days: int = 30) -> List[str]:
        """
        성과가 좋은 프롬프트 조회

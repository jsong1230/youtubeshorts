...
if __name__ == "__main__":
    ...
    parser.add_argument('--type', type=str, help='콘텐츠 타입 (hook, quote, story, fact, etc.)')
    parser.add_argument('--force', action='store_true', help='강제 업로드 (중복 체크 무시)')
    parser.add_argument('--only-create', action='store_true', help='영상 생성만 하고 업로드하지 않음')
    parser.add_argument('--scheduler', action='store_true', help='스케줄러 모드로 실행')
    parser.add_argument('--lang', type=str, help='언어 (ko, en)')
    parser.add_argument('--auto', action='store_true', help='자동 업로드 (사용자 확인 생략)')

...
class VideoPipeline:
    ...
    def run(
        ...
        try:
            ...
            if force:
                logger.warning("⚠️ Force Mode: Skipping duplicate checks")
            logger.info("=" * 50)
            ...
            # 4. Generate content
            video_assets = self._generate_content(
                topic, content_type, language, performance_prompt
            )
    ...
    def _check_constraints(self, force: bool) -> bool:
        ...
        if self.sync_manager.check_today_uploaded():
            ...
            logger.warning("⚠️ Already uploaded today (Local Check).")
        ...
        if not self.use_multi_platform:
            if hasattr(self.uploader, "check_today_uploaded"):
                if self.uploader.check_today_uploaded():
                    logger.warning("⚠️ Already uploaded today (API Check).")
                    return False
    ...
    def _get_performance_prompt(self) -> str:
        ...
        try:
            top_videos = self.database.get_top_performing_videos(
                limit=3, days=30, min_views=50
            )
            ...
            if top_topics:
                topics_text = ", ".join(
                    [t["topic"] for t in top_topics if t.get("topic")]
                )
                if topics_text:
                    additions.append(
                        f"Recent top topics: {topics_text}. Use similar style/tone but offer new perspectives."
                    )

            if top_videos:
                avg_engagement = sum(
                    v.get("engagement_rate", 0) for v in top_videos
                ) / len(top_videos)
                if avg_engagement > 2.0:
                    additions.append(
                        "Recent high engagement features: Clear practical info, curiosity-inducing structure, actionable tips."
                    )
    ...
    def _generate_content(
        ...
        if thumbnail_path:
            ...
            self.video_generator.image_generator.embed_thumbnail_frame(
                video_path, thumbnail_path
            )

        # Description
        channel_info = getattr(self.uploader, "get_channel_info", lambda: None)()
        recent_videos: list = getattr(
            self.uploader, "get_recent_videos", lambda **k: []
        )(max_results=3)
    ...
    def _update_records(
        self, assets, upload_results, request_id, content_type, performance_prompt
    ):
    ...
    def _append_to_history(self, log_entry: Dict[str, Any]):
        ...
        try:
            ...
            upload_date = datetime.fromisoformat(log_entry["timestamp"]).strftime(
                "%Y-%m-%d"
            )
...
"""
            ...
            if "## Recent Updates" in content:
                content = content.replace(
                    "## Recent Updates", f"## Recent Updates\n{history_entry}"
                )
            ...
            logger.info("✅ Appended to HISTORY.md")

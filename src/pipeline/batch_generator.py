...
class BatchVideoGenerator:
    ...
    def generate_batch(
        ...
        if topics:
            if len(topics) < count:
                logger.warning(
                    f"⚠️ 주제가 부족합니다 ({len(topics)} < {count}). 나머지는 자동 생성됩니다."
                )
        ...
        for i in range(count):
            ...
            try:
                result = self._generate_single_video(
                    index=index, topic=topic, upload=upload, force=force
                )
                ...
                self.results.append({"index": index, "success": False, "error": str(e)})
    ...
    def _generate_single_video(
        ...
        try:
            ...
            return {"index": index, "success": False, "error": error_msg}

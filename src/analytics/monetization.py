...
class MonetizationTracker:
    ...
    def update_video_stats(self, video_id: str, uploader=None):
        ...
        # 영상 찾기
        video = next(
            (v for v in self.data["videos"] if v["video_id"] == video_id), None
        )
        if not video:
            return
    ...
    def get_progress_report(self) -> Dict:
        ...
        if self.data["videos"]:
            first_video_date = datetime.fromisoformat(
                self.data["videos"][0]["upload_date"]
            )
    ...
    def print_report(self):
        ...
        logger.info(
            f"  목표 범위: ${report['target_revenue_range']['min']}-${report['target_revenue_range']['max']}"
        )

...
class ThumbnailOptimizer:
    ...
    def _init_database(self):
        ...
        # 인덱스 생성
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_video_id ON thumbnails(video_id)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_variant ON thumbnails(thumbnail_variant)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_ctr ON thumbnails(ctr)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_upload_date ON thumbnails(upload_date)')
    ...
    def add_thumbnail(
        ...
        try:
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (video_id, thumbnail_path, thumbnail_variant, thumbnail_style, now, now, now))
    ...
    def update_thumbnail_stats(
        ...
        try:
            ...
            if row:
                ...
                new_impressions = impressions if impressions is not None else old_impressions
                ...
                cursor.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                ''', (video_id, views or 0, impressions or 0, ctr, views or 0, now, now, now))

...
class PerformancePredictor:
    ...
    def extract_features(self, video_data: Dict) -> Dict:
        ...
        features['is_finance'] = 1 if any(k in topic for k in ['money', 'finance', 'invest', 'budget', 'save', '돈', '재태크', '투자']) else 0
        features['is_productivity'] = 1 if any(k in topic for k in ['productivity', 'habit', 'routine', 'time', '생산성', '습관']) else 0
        features['is_lifestyle'] = 1 if any(k in topic for k in ['life', 'lifestyle', 'health', 'wellness', '라이프', '건강']) else 0
    ...
    def predict_performance(
        ...
        try:
            ...
            # 2. 콘텐츠 타입별 평균 성과 계산
            content_type = video_data.get('content_type', 'auto')
            content_type_avg = self._calculate_content_type_average(content_type, historical_data)
            ...
            # 예측 결과 저장
            self._save_prediction(video_data.get('video_id'), features, {
    ...
    def _calculate_topic_average(self, topic: str, historical_data: List[Dict]) -> Dict:
        """주제별 평균 성과 계산"""
        topic_videos = [v for v in historical_data if topic.lower() in v.get('topic', '').lower()]
    ...
    def _calculate_content_type_average(self, content_type: str, historical_data: List[Dict]) -> Dict:
        """콘텐츠 타입별 평균 성과 계산"""
    ...
    def _calculate_time_average(self, upload_hour: int, historical_data: List[Dict]) -> Dict:
        """업로드 시간별 평균 성과 계산"""
    ...
    def _calculate_style_average(self, video_style: str, historical_data: List[Dict]) -> Dict:
        """스타일별 평균 성과 계산"""
        style_videos = [v for v in historical_data if v.get('video_style') == video_style]
    ...
    def update_prediction_accuracy(self, video_id: str, actual_views: int):
        ...
        try:
            ...
            if row:
                ...
                error_rate = abs(predicted_views - actual_views) / max(actual_views, 1) * 100
        ...
        try:
            ...
            for video in historical_data:
                try:
                    ...
                    hour_performance[hour]['engagement_rate'].append(video.get('engagement_rate', 0))
                except:
                    continue
...
class CompetitorAnalyzer:
    ...
    def benchmark_against_competitors(self, competitor_data: List[Dict]) -> Dict:
        ...
        try:
            ...
            # 경쟁사 평균 계산
            competitor_avg = {
                'avg_views': np.mean([c.get('avg_views_per_video', 0) for c in competitor_data]),
                'avg_engagement_rate': np.mean([c.get('avg_engagement_rate', 0) for c in competitor_data]),
                'upload_frequency': np.mean([c.get('upload_frequency', 0) for c in competitor_data])
            }
            
            # 비교 분석
            comparison = {
                'views_performance': 'above' if own_data['avg_views'] > competitor_avg['avg_views'] else 'below',
                'engagement_performance': 'above' if own_data['avg_engagement_rate'] > competitor_avg['avg_engagement_rate'] else 'below',
                'upload_frequency_performance': 'above' if own_data['upload_frequency'] > competitor_avg['upload_frequency'] else 'below',
                'gap_analysis': {
                    'views_gap': own_data['avg_views'] - competitor_avg['avg_views'],
                    'engagement_gap': own_data['avg_engagement_rate'] - competitor_avg['avg_engagement_rate'],
                    'frequency_gap': own_data['upload_frequency'] - competitor_avg['upload_frequency']
                }
            }
    ...
    def _get_own_channel_data(self) -> Dict:
        """자체 채널 데이터 가져오기"""
        try:
            historical_data = self.video_db.get_top_performing_videos(limit=100, days=30)
    ...
    def _generate_benchmark_recommendations(self, comparison: Dict) -> List[str]:
        ...
        if comparison['engagement_performance'] == 'below':
            recommendations.append("참여율 향상을 위해 콘텐츠 품질 개선 및 CTA 강화 필요")
...
class AudienceSegmentAnalyzer:
    ...
    def _analyze_content_type_segments(self, days: int) -> List[Dict]:
        ...
        try:
            # 콘텐츠 타입별 데이터 가져오기
            content_types = ['hook', 'quote', 'story', 'fact', 'short_story', 'meditation', 'breathing']
        ...
        try:
            ...
            for group_name, (start_hour, end_hour) in time_groups.items():
                ...
                if group_videos:
                    ...
                    engagement_rates = [v.get('engagement_rate', 0) for v in group_videos]

...
class GoogleTrendsCollector:
    ...
    def __init__(self):
        ...
        self.min_request_interval = (
            2.0  # 최소 요청 간격 (초) - Google Trends rate limiting 방지
        )
    ...
    def get_trending_keywords(
        ...
        try:
            ...
            self.pytrends.build_payload(
                kw_list=keywords_to_check, timeframe=timeframe, geo=geo
            )
            ...
            logger.info(
                f"✅ Google Trends에서 {len(trending_keywords)}개 키워드 트렌드 수집"
            )
    ...
    def get_trending_keywords_by_category(
        self, category: str = "finance", timeframe: str = "today 7-d", geo: str = "US"
    ) -> List[Dict]:
        """
        카테고리별 트렌드 키워드 가져오기
    ...
    def convert_keywords_to_topics(
        ...
        if not self.openai_client:
            logger.warning(
                "⚠️ OpenAI API가 없어 Google Trends 키워드를 주제로 변환할 수 없습니다."
            )
            # 간단한 변환만 수행
            topics = []

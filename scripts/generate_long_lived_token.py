...
def update_env_file(env_path, long_token, app_id):
    ...
    if app_id:
        ...
        if re.search(pattern_app_id, content, re.MULTILINE):
            content = re.sub(
                pattern_app_id, replacement_app_id, content, flags=re.MULTILINE
            )
...
def main():
    ...
    try:
        long_token, expires_in = generate_long_lived_token(
            short_token, app_id, app_secret
        )
        expires_in_days = expires_in // 86400

        print("   ✅ Long-lived Token 생성 성공!")
        ...
        if update_env_file(env_path, long_token, app_id):
            ...
            print("\n" + "=" * 60)
            print("✅ 완료!")
            print("   Long-lived Token이 .env 파일에 저장되었습니다.")
            print(
                f"   유효 기간: {expires_in_days}일 (만료일: {expires_date.strftime('%Y-%m-%d')})"
            )
            print("=" * 60)
        else:

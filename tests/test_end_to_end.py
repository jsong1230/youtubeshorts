...
class TestEndToEnd(unittest.TestCase):
    ...
    def test_pipeline_execution(self):
        ...
        with patch(
            "src.pipeline.bot.AIVideoGenerator", return_value=self.mock_video_generator
        ), patch(
            "src.pipeline.bot.YouTubeUploader", return_value=self.mock_uploader
        ), patch(
            "src.pipeline.bot.VideoDatabase", return_value=self.mock_database
        ), patch(
            "src.pipeline.bot.SyncManager", return_value=self.mock_sync_manager
        ), patch(
            "src.pipeline.bot.UserRequestHandler",
            return_value=self.mock_user_request_handler,
        ), patch(
            "src.pipeline.bot.NotificationService",
            return_value=self.mock_notification_service,
        ), patch(
            "src.pipeline.bot.MonetizationTracker", return_value=self.mock_monetization
        ), patch(
            "src.pipeline.bot.MultiPlatformUploader"
        ), patch(
            "src.pipeline.bot.ABTestDatabase"
        ), patch(
            "src.pipeline.bot.TopicDatabase"
        ), patch(
            "src.pipeline.bot.ThumbnailOptimizer"
        ), patch(
            "src.pipeline.bot.PerformancePredictor"
        ), patch(
            "src.pipeline.bot.AutoOptimizer"
        ), patch(
            "src.pipeline.bot.CompetitorAnalyzer"
        ), patch(
            "src.pipeline.bot.AudienceSegmentAnalyzer"
        ), patch(
            "src.pipeline.bot.SeriesGenerator"
        ), patch(
            "src.pipeline.bot.CommentAnalyzer"
        ):
    ...
    def test_pipeline_constraints(self):
        ...
        with patch(
            ...
            video_id = bot.create_and_upload(
                topic="Test Topic", force=False, auto_upload=True
            )

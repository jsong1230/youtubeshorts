...
class TestTrendCollector:
    ...
    def trend_collector(self):
        ...
        with patch(
            "src.analytics.trend_collector.get_authenticated_service"
        ) as mock_auth, patch(
            "src.analytics.trend_collector.OpenAI"
    ...
    def test_init_without_apis(self, monkeypatch):
        ...
        # Patch the settings object attribute directly
        with patch(
            "src.analytics.trend_collector.get_authenticated_service",
            ...
            assert (
                collector.openai_client is None or collector.openai_client.api_key == ""
            )
            ...
            "items": [
                {
                    ...
                    "snippet": {"tags": ["test", "video", "shorts"]},
                }
            ]
        }
            ...
            "items": [
                {
                    ...
                    "snippet": {"tags": ["finance", "money", "tips"]},
                }
            ]
        }
        trend_collector.youtube.videos.return_value = mock_videos
    ...
    def test_extract_keywords_from_videos(self, trend_collector):
        ...
        keywords = trend_collector.extract_keywords_from_videos(
            videos, min_views=10000, top_n=10
        )
        ...
        assert (
            "money" in keyword_text
            or "save" in keyword_text
            or "financial" in keyword_text
        )
    ...
    def test_refine_keywords_with_ai(self, trend_collector):
        ...
        mock_response.choices[0].message.content = (
            "money, finance, investment, savings, tips"
        )
        trend_collector.openai_client.chat.completions.create.return_value = (
            mock_response
        )
    ...
    def test_refine_keywords_with_ai_error(self, trend_collector):
        ...
        # Mock OpenAI error
        trend_collector.openai_client.chat.completions.create.side_effect = Exception(
            "API Error"
        )
    ...
    def test_cpm_keywords(self, trend_collector):
        ...
        assert (
            TrendCollector.CPM_KEYWORDS["insurance"]
            > TrendCollector.CPM_KEYWORDS["finance"]
        )
    ...
    def test_get_trending_keywords_integration(self, trend_collector):
        ...
        trend_collector.get_trending_shorts = Mock(
            return_value=[
                {
        ...
        trend_collector.extract_keywords_from_videos = Mock(
            return_value=["money", "finance", "savings", "tips"]
        )

        # Use the actual flow: get_trending_shorts -> extract_keywords_from_videos
        videos = trend_collector.get_trending_shorts(max_results=10)
        keywords = trend_collector.extract_keywords_from_videos(
            videos, min_views=10000, top_n=5
        )

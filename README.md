...
- **배경 영상/이미지**: Pexels Video API를 활용한 CC0 라이선스 배경 영상 또는 무료 이미지 자동 삽입
  ...
  - **힐링 키워드 모드 (기본 활성화)**: 오디오는 정보성, 비디오는 항상 힐링/자연/동물/Satisfying 영상만 사용 (스크립트 주제와 무관)
  - **중복 방지 시스템**: 한번 사용한 영상은 `data/used_video_ids.json`에 기록되어 다른 영상에서 재사용되지 않음
  - **최신 영상 우선**: Pexels API의 `sort=newest` 파라미터로 항상 최신 영상을 우선 검색
- **주제 중복 방지**: 업로드된 주제는 자동으로 저장되어 동일하거나 유사한 주제의 중복 생성을 방지
  - 최근 1년간 업로드된 주제와 비교하여 유사도 70% 이상이면 중복으로 판단
  - `data/upload_log.json`과 `data/videos.db`에서 기존 주제를 자동으로 수집
  - 사용자가 직접 주제를 제공할 때도 자동으로 중복 체크 수행
  - `--force` 플래그로 중복 체크를 건너뛰고 강제 생성 가능
...
시스템은 업로드된 모든 주제를 자동으로 추적하여 동일하거나 유사한 주제의 중복 생성을 방지합니다.

**작동 방식:**
- 최근 1년간 업로드된 주제를 `data/upload_log.json`과 `data/videos.db`에서 자동으로 수집
- 새 주제가 기존 주제와 유사도 70% 이상이면 중복으로 판단하여 생성 중단
- 정확히 일치하거나 포함 관계인 경우도 중복으로 판단
- 자동 주제 생성 시와 사용자가 직접 주제를 제공할 때 모두 작동
...
**강제 생성:**
중복 주제를 강제로 생성하려면 `--force` 플래그를 사용하세요:
```bash
python main.py test "중복된 주제" --force
```

### 배경 영상 중복 방지가 너무 강력한 경우

배경 영상 중복 방지 시스템이 너무 강력해서 영상을 만들 수 없는 경우:

1. **현재 사용된 ID 개수 확인**:
   ```bash
   python main.py check-video-ids
   ```

2. **사용 기록 초기화** (모든 사용 기록 삭제):
   ```bash
   python main.py reset-video-ids
   ```
   또는 확인 없이 바로 초기화:
   ```bash
   python main.py reset-video-ids --yes
   ```

3. **권장 사항**:
   - 사용된 ID가 100개 이상이면 초기화를 고려하세요
   - 초기화 후에는 이전에 사용한 영상도 다시 사용 가능합니다
   - Pexels에 새로운 영상이 계속 업로드되므로 주기적으로 초기화해도 문제없습니다

...
- **하루 6개 업로드**: YouTube Shorts 하루 최대 제한(6개)까지 자동으로 영상 생성 및 업로드
...
### 4. 스케줄링
- **자동 스케줄러**: 매일 지정된 시간에 6개 콘텐츠 타입을 모두 한 번에 생성 및 업로드
- **6개 콘텐츠 타입 자동 생성**: HOOK, QUOTE, STORY, FACT, SHORT_STORY, AUTO 타입을 순차적으로 생성
...
# 업로드 스케줄 설정
UPLOAD_SCHEDULE_TIME=09:00  # 매일 이 시간에 6개 콘텐츠 타입 모두 생성 및 업로드
...
```bash
# 스케줄러 시작 (하루 6개 자동 업로드)
python main.py schedule
```

스케줄러는 매일 지정된 시간(기본값: 09:00, 한국 시간)에 6개 콘텐츠 타입(HOOK, QUOTE, STORY, FACT, SHORT_STORY, AUTO)을 모두 한 번에 생성하고 업로드합니다.
각 타입별로 계절에 맞는 주제가 우선적으로 선택됩니다.
...
- **업로드 시간**: `UPLOAD_SCHEDULE_TIME=09:00` (24시간 형식, 이 시간에 6개 콘텐츠 타입 모두 생성)

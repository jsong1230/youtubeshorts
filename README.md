...
- **영어 설명/태그 자동 생성**: Shorts용 영어 설명 및 태그 자동 작성 (필요 시 한글 문구 추가 가능)
- **최적화된 태그**: 검색 최적화를 위한 영어/한글 혼합 태그 자동 설정
...
DEFAULT_DESCRIPTION=AI-generated YouTube Shorts that deliver one practical money or routine insight every day. Subscribe for daily focus resets.
DEFAULT_TAGS=shorts,ai,automation,money,tinyhabits,productivity
```
...
2025년 업데이트된 가이드는 **하루 1개 고품질 전략**과 **트렌드 가중치 시스템**을 중심으로 동작합니다.

- ✅ **Hook**: “Your spending isn’t random, it’s rehearsed.”처럼 3초 안에 임팩트 전달
- ✅ **Quote**: 명언/인사이트 한 줄 → 실생활 연결
- ✅ **Story**: Hook → 사건 → 교훈 → 결론 구조의 1분 스토리
- ✅ **Fact**: 팩트 → 왜 중요한지 → 실천법
- ✅ **Short Story**: AI 이미지 기반 초단편 스토리
- ✅ **Auto**: `TREND_MODE` 가중치(글로벌 40% / 계절 25% / 성과 20% / 탐색 15%) 기반 자동 선택

### 주제 카테고리 & 계절 로직
- 🌤 계절: 봄 정리 → 여름 전기요금 → 가을 리셋 → 겨울 난방비
- 🏠 생활 최적화: 디클러터링, 냉장고·옷장 리셋, 책상 정리
- 💰 재테크/루틴: 소비 패턴 분석, 난방비 절약 루틴, 연말 회고
- 🧠 자기계발: 루틴, 1% 개선, 감정 관리
- 🚨 리스크: 예상치 못한 지출, 보험, 자동차 관리

계절 주제는 25% 확률로 우선 선택되고, 나머지는 글로벌 트렌드·성과·탐색 가중치에 따라 자동 선택됩니다. 확장 주제팩과 프롬프트 세부 내용은 [docs/CONTENT_OPTIMIZATION.md](./docs/CONTENT_OPTIMIZATION.md)에 정리되어 있습니다.

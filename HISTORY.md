...
- **2025-12-10 - 동기부여/힐링 콘텐츠 테스트 영상 업로드**
  - **테스트 영상 생성 및 비공개 업로드**:
    - 한국어 영상: "운이 없다고 느끼시나요? 긍정적인 생각이 당신의 운을 바꿔줄 수 있습니다!" (46.51초)
      - Video ID: `MC64RwBPr-8`
      - URL: <https://www.youtube.com/watch?v=MC64RwBPr-8>
      - 파일: `output/videos/shorts_20251210_224458.mp4`
      - 비공개 업로드 완료
    - 영어 영상 (캐나다): "Tiny routines create massive peace." (53.90초)
      - Video ID: `H1x2G_Sr_U8`
      - URL: <https://www.youtube.com/watch?v=H1x2G_Sr_U8>
      - 파일: `output/videos/shorts_20251210_224456.mp4`
      - 비공개 업로드 완료
  - **주제 선정**: 한국과 캐나다 정서에 맞는 동기부여/힐링 주제 선정
    - 한국어 주제: "운이 없다고 느끼시나요? 긍정적인 생각이 당신의 운을 바꿔줄 수 있습니다!"
    - 영어 주제: "Tiny routines create massive peace."
...
- **2025-12-10 - 동기부여 및 힐링 콘텐츠로 전면 전환**
  - **콘텐츠 방향 전면 변경**: 실용 정보 중심 → 동기부여 및 힐링 콘텐츠로 전환
    ...
    - 사람들의 마음을 울리고 통찰력을 주는 콘텐츠 생성
    - 철학적/심리학적 관점에서 문제의 원인을 짚어주는 구조
  - **프롬프트 구조 변경**: 
    - 후킹(0-3초) → 전개(본론) → 핵심 메시지(해결책) → 마무리(여운) 구조
    - 차분하고 다정하지만 단호한 통찰력이 느껴지는 '성우' 톤
    - 구어체 사용 (자연스러운 말투)
    - 공감적이고 이해하는 태도로 시청자의 아픔에 공감
  - **영상 길이 변경**: 
    - Hook 타입: 20초 → 50초 (45-55초 범위)
    - Auto 타입: 22초 → 50초
    - 동기부여 및 힐링 콘텐츠에 적합한 길이로 조정
  - **배경 영상 폴백 개선**: 
    - Pexels API 실패 시 단색 배경으로 폴백
    - 동기부여/힐링 콘텐츠에 맞는 차분한 어두운 배경 사용
  - **테스트 영상 생성**:
    - 한국어 영상: "쇼펜하우어의 인생 조언" (51.57초)
      - 파일: `output/videos/shorts_20251210_065350.mp4`
      - 단색 배경 사용 (Pexels API 실패로 폴백)
